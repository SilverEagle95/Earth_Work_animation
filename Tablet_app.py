...
def todo_app():
    ...
    def add_task():
        ...
        if task:
            ...
            messagebox.showwarning("Hiba", "Kérem adjon meg egy feladatot!")
    ...
    def delete_task():
        try:
            ...
            messagebox.showwarning("Hiba", "Kérem válasszon ki egy feladatot törléshez!")
    ...
    def mark_done():
        try:
            ...
            messagebox.showwarning("Hiba", "Kérem válasszon ki egy feladatot a megjelöléshez!")

    # Tkinter ablak létrehozása
    root = tk.Tk()
    root.title("Todo Alkalmazás")

    # Feladat hozzáadás rész
...
def beginner_infotk():
    ...
    def show_info():
        ...
        print = függvény
        input = függvény
        str(string) = karakterlánc változtatása & típus
        int(integer) = szám típus
        import = kulcsszó
        lower = függvény
        find = függvény
        return = kulcsszó

        Szeretnél egy projektet ezekkel?: i/n:
        """
        messagebox.showinfo("Információk", info_text)
    ...
    def run_project():
        ...
        A kód, amit szerkeszthetsz: Szia {input_name} vagy Szia

        variable_str = str(8)
        A kód: 81, mert a 'variable' egy típus, nem pedig egy szám

        variable_int = int(8)
        A kód: 9, mert az int típus átalakítja a 'str'-t egy számmá

        import time
        time.sleep(1)
        print('Hi')
        Várj 1 másodpercet, majd futtasd le a kódot: Hi, de lehet 'import random' vagy valami más is
        ...
        A 'return' különleges kódokat ad vissza
        """
        messagebox.showinfo("Projekt Kód", result_text)

    def on_exit():
        root.destroy()

    root = tk.Tk()
    root.title("Programozási Alapok")

    info_button = tk.Button(root, text="Információk", command=show_info)
    info_button.pack(pady=10)

    project_button = tk.Button(root, text="Projekt Futása", command=run_project)
    project_button.pack(pady=10)

    exit_button = tk.Button(root, text="Kilépés", command=on_exit)
...
def weather_app():
    ...
    class WeatherApp:
        def __init__(self, root):
            self.root = root
            self.root.title("Időjárás alkalmazás")
        ...
        def create_widgets(self):
            # Cím
            self.label_title = tk.Label(self.root, text="Időjárás alkalmazás", font=("Helvetica", 20, "bold"),
                                        fg="#333")
            self.label_title.grid(row=0, column=0, columnspan=2, pady=10)

            # Időjárás lekérdezése gomb
            self.button_get_weather = tk.Button(self.root, text="Időjárás lekérdezése", command=self.get_weather,
            ...
            self.tree = ttk.Treeview(self.root, columns=("Dátum", "Időjárás", "Hőmérséklet", "Páratartalom"),
                                     show="headings")
            self.tree.heading("Dátum", text="Dátum")
            self.tree.heading("Időjárás", text="Időjárás")
            self.tree.heading("Hőmérséklet", text="Hőmérséklet (°C)")
            self.tree.heading("Páratartalom", text="Páratartalom (%)")
            self.tree.column("Dátum", width=100)
            self.tree.column("Időjárás", width=150)
            self.tree.column("Hőmérséklet", width=120)
            self.tree.column("Páratartalom", width=120)
        ...
        def get_weather(self):
            try:
                ...
                print(e)
                messagebox.showerror("Hiba", "Nem sikerült lekérdezni az időjárást.")
...
def white_text():
    ...
    def main():
        ...
        pygame.display.set_caption("3D Szöveg Effekt")
...
def Luck_game():
    ...
    class LuckyGame:
        def __init__(self, master):
            self.master = master
            self.master.title("Szerencse Játék")
            ...
            self.button_spin = Button(master, text="Pörgetés", command=self.spin)
        ...
        def spin(self):
            try:
                ...
                self.label_result.config(text="Érvénytelen tét! Adj meg egy számot.")
                return

            if self.bet > self.points:
                self.label_result.config(text="Nincs elég pontod a tétre!")
            else:
                self.points -= self.bet
                self.label_result.config(text="Pörgetés folyamatban...")
                ...
                if emojis[0] == emojis[1] == emojis[2]:
                    ...
                    self.label_result.config(text=f"Gratulálok! Tripla! Pontszám duplázva! Pontok: {self.points}")
                elif emojis[0] == emojis[1] or emojis[0] == emojis[2] or emojis[1] == emojis[2]:
                    self.points += self.bet
                    self.label_result.config(text=f"Kettes pár! Pontszám növelve! Pontok: {self.points}")
                elif emojis.count(emojis[0]) == 3 and emojis[0] == emojize(":pear:"):
                    self.points += self.bet * 5
                    self.label_result.config(
                        text=f"Nyertél egy extra nagyobbat! Pontszám ötszörözve! Pontok: {self.points}")
                else:
                    self.label_result.config(text=f"Sajnos nem nyertél! Pontok: {self.points}")
...
def codeeditor():
    ...
    def run_code():
        code = code_entry.get("1.0", tk.END)  # Az input mezőből lekérjük a kódot
        try:
            exec(code)  # Futtatjuk a kódot
        except Exception as e:
            messagebox.showerror("Hiba", f"Hiba történt a kód futtatása közben:\n{str(e)}")
...
def text_to_speech():
    ...
    # Lejátszás gomb
    gomb = tk.Button(ablak, text="Lejátszás", command=lejatszas)
    gomb.pack(pady=10)
...
def italy_translate():
    ...
    italian_to_hungarian = {
        ...
        "Én 10 éves vagyok": "Ho dieci anni",
        # ... (a szótár tartalma maradt ugyanaz)
    }
    ...
    class ItalianLanguageApp:
        def __init__(self, root):
            ...
            self.label = tk.Label(root, text="Válassz egy olasz szót:", font=("Helvetica", 16))
            ...
            self.translate_button = tk.Button(root, text="Fordítás", command=self.translate)
            self.translate_button.pack(pady=10)

        def translate(self):
            italian_word = self.word_choice.get()
            italian_translation = italian_to_hungarian.get(italian_word, "Nincs fordítás.")

            if italian_translation != "Nincs fordítás.":
                try:
                ...
                messagebox.showinfo("Fordítás", "Nincs elérhető fordítás.")
...
def base64_encoder():
    ...
    def kodolas():
        ...
        kodolt_eredmeny.config(text="Kódolt szöveg: " + kodolt_szoveg)
    ...
    kodolo_gomb = tk.Button(ablak, text="Kódolás és Másolás", command=kodolas)
    kodolo_gomb.pack()
